...
### Added
- **Galeries d'images — socle (BL-069)** : un dossier contenant plus de 3 images et aucune vidéo (sous-dossiers comptés récursivement) est désormais traité comme une **galerie** — un média unique lu page par page, au lieu d'un dossier à parcourir. L'ouvrir affiche la première image (ou la reprise) ; la position est sauvée sur le dossier et restaurée à la réouverture. Dans la liste, la galerie apparaît comme un média (icône 🖼️, barre de progression, état non vu / en cours / vu). Nouvel endpoint `GET /api/gallery/list` (séquence aplatie : profondeur d'abord, fichiers avant sous-dossiers, tri naturel) ; `media_type: "gallery"` exposé par `/api/files`.
