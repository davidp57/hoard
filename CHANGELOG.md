...
- **Gamepad — curseur à -1 après auto-play post-suppression (BL-052)** : après suppression d'un fichier en plein écran, le fichier suivant démarrait correctement via `_autoPlayNextFullscreen`, mais `playVideo` appelait ensuite `renderFiles()` sans `_gpPendingRestoreIdx` défini, réinitialisant `_gpCursorIdx = -1`. Résultat : commandes buggées, vidéo fantôme possible (BL-044 regression), curseur à -1 à la sortie du plein écran. Corrigé en sauvegardant la position de la vidéo en cours dans `_gpPendingRestoreIdx` avant le `renderFiles()` final de `playVideo`.
